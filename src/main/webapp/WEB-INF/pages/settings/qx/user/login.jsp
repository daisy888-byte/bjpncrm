	...
	<script type="text/javascript">
		$(function (){
			...
			// $("#loginbtn").click(function (){
				...
				var isRemAct = $("#isRemAct").prop("checked")
				if(loginAct==""){
					alert("用户名不能空！")
					return
				}
				...
				$.ajax({
					...
					data:{"loginAct":loginAct,"loginPwd":loginPwd,"isRemAct":isRemAct},
					dataType:"json",
					type:"post",
			...
			<form action="workbench/index.html" class="form-horizontal" role="form">
				<div class="form-group form-group-lg">
					...
					<div style="width: 350px; position: relative;top: 20px;">
						...
						<c:if test="${not empty cookie.loginPwd}">
					...
					<div class="checkbox" style="position: relative;top: 30px; left: 10px;">
						<label>
							<c:if test="${not empty cookie.loginAct and not empty cookie.loginPwd}">
								<input type="checkbox" id="isRemAct" checked>
							</c:if>
							<c:if test="${empty cookie.loginAct or empty cookie.loginPwd}">
								<input type="checkbox" id="isRemAct" >
							</c:if>
							十天内免登录
						</label>
						&nbsp;&nbsp;
					...
					<button type="button" id="loginbtn"  style="width: 350px; position: relative;top: 45px;">登录</button>
				</div>
			</form>
		</div>
	</div>
